...
void my_kernel_function (input_stream<read_type>* restrict input_1, input_stream<read_type>* restrict input_2, output_stream<write_type>* restrict output) {
    ...
    aie::vector<func_type, vector_size> to_write1 = convert_to_func_type(size_vec1);
    aie::vector<func_type, vector_size> to_write2 = convert_to_func_type(size_vec2);

    float sum = aie::reduce_add(to_write1) + aie::reduce_add(to_write2);
    ...
    writeincr(output, (write_type) final_sum / size1); // divide and write to get the real MSE
    */
}