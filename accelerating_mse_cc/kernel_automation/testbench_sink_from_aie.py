...
float rmse(int size, float *img_ref, float *img_float){
    unsigned long long int tot = 0;
    for (int i = 0; i < size; i++){
        tot += (img_ref[i] - img_float[i]) * (img_ref[i] - img_float[i]);
    }
    return sqrt((float) tot / size);
}
...
'''
    ...
    real_values[0] = rmse(input_size, img_ref, img_float);
    ...
    if (abs(buffer[0] - real_values[0]) / real_values[0] > 0.01){{
        error = true;
        std::cout << "value of rmse is wrong, iteration number: " << 0 << " --> got " << buffer[0] << " expected " << real_values[0] << std::endl;
    }}
    else{{
        std::cout << "value of rmse is right, iteration number: " << 0 << ", value = " << buffer[0] << std::endl;
    }}
    if(!error){{
        std::cout << "Test Passed!" << std::endl;
    }}
