...
extern "C" {
void setup_aie(int size1, int size2, hls::burst_maxi<ap_uint<128>> input_1, hls::burst_maxi<ap_uint<128>> input_2, hls::stream<stream_type>& s_1, hls::stream<stream_type>& s_2, hls::stream<stream_type>& s_3, hls::stream<stream_type>& s_4) {
	#pragma HLS interface m_axi port=input_1 offset=slave bundle=gmem0 max_read_burst_length=256
	...
	for (int i = 0; i < burst_count; i++){
		...
		for (int j = 0; j < burst_length/kernel_count; j++){
			...
			buf_1[i] = input_1.read();
			buf_2[i] = input_2.read();
			buf_3[i] = input_1.read();
			buf_4[i] = input_2.read();
		}
		for (int j = 0; j < burst_length/kernel_count; j++){
			s_1.write((stream_type) buf_1[i]);
			s_2.write((stream_type) buf_2[i]);
			s_3.write((stream_type) buf_3[i]);
			s_4.write((stream_type) buf_4[i]);
		}
	}
