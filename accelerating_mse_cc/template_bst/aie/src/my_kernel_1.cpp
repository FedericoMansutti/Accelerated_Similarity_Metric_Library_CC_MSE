...
void my_kernel_function (input_stream<read_type>* restrict input_1, input_stream<read_type>* restrict input_2, output_stream<write_type>* restrict output) {
    ...
    writeincr(output, (write_type) (final_num) / (final_denom));
}

/*void convert_to_func_type(const aie::vector<read_type, vector_size>& src_1, const aie::vector<read_type, vector_size>& src_2, int coeff, aie::vector<func_type, vector_size>& dst_1, aie::vector<func_type, vector_size>& dst_2, aie::vector<func_type, vector_size>& dst_3) {
    int num;
    for (int i = 0; i < vector_size; ++i) {
        #pragma HLS UNROLL factor=16
        #pragma HLS pipeline II=1
        num = static_cast<func_type>(src_1[i]) + coeff;
        dst_2[i] = static_cast<func_type>(src_2[i]);
        dst_3[i] = num * num;
        dst_1[i] = dst_3[i] * dst_2[i];
    }
}
...
void my_kernel_function (input_stream<read_type>* restrict input_1, input_stream<read_type>* restrict input_2, output_stream<write_type>* restrict output) {
    aie::vector<size_type, vector_size> size_vec1 = readincr_v<vector_size>(input_1);
    aie::vector<size_type, vector_size> size_vec2 = readincr_v<vector_size>(input_2);
    ...
    #pragma HLS ARRAY_PARTITION complete variable=partial_num
    aie::vector<func_type, vector_size> partial_num[num_partitions]; // init partial sums which contains the "map" phase of map-reduce pattern
    #pragma HLS ARRAY_PARTITION complete variable=partial_denom
    aie::vector<func_type, vector_size> partial_denom[num_partitions]; // init partial sums which contains the "map" phase of map-reduce pattern
    for (int i = 0; i < num_partitions; i++){
        #pragma HLS UNROLL factor=16
        #pragma HLS pipeline II=1
    ...
    int mean_1 = readincr_v<vector_size>(input_1)[0];
    int neg_mean_1 = mean_1 * -1;
    ...
    for(int i = 0; i < size1 / vector_size; i++){
        ...
        #pragma HLS pipeline II=1
        convert_to_func_type(readincr_v<vector_size>(input_1), readincr_v<vector_size>(input_2), neg_mean_1, vec_1, vec_2, vec_3);

        partial_num[i % num_partitions] = aie::add(partial_num[i % num_partitions], vec_1);
        partial_denom[i % num_partitions] = aie::add(partial_denom[i % num_partitions], vec_3);
    }

    long double final_num = 0.0;
    long double final_denom = 0.0;
    for (int i = 0; i < num_partitions; i++) {
