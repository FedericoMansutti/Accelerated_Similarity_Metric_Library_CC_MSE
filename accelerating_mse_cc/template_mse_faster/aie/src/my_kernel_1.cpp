...
void my_kernel_function (input_stream<read_type>* restrict input_1, input_stream<read_type>* restrict input_2, output_stream<write_type>* restrict output) {
    ...
    aie::vector<func_type, vector_size> diff_squared;
    aie::vector<func_type, vector_size> img1;
    aie::vector<func_type, vector_size> img2;
    // Process each vector with partial 

    const int loop_size = size1 / vector_size;

    for (int i = 0; i < loop_size; i++) 
    chess_loop_range(4, )
    chess_prepare_for_pipelining
    {
        img1 = convert_to_func_type(readincr_v<vector_size>(input_1));
        img2 = convert_to_func_type(readincr_v<vector_size>(input_2));

        diff = aie::add(img1, aie::neg(img2)); // compute difference vectorized
        diff_squared = aie::mul(diff, diff).to_vector<func_type>();
        partial_sums[i % num_partitions] = aie::add(partial_sums[i % num_partitions], diff_squared); // add to partial sums, after conversion from accumulator to vector
    }
    ...
    write_type res = final_sum / size1;
