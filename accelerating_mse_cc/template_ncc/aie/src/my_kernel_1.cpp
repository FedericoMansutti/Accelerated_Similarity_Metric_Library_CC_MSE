...
void my_kernel_function (input_stream<read_type>* restrict input_1, input_stream<read_type>* restrict input_2, output_stream<write_type>* restrict output) {
    ...
    aie::vector<func_type, vector_size> partial_denom_1[num_partitions]; // init partial sums which contains the "map" phase of map-reduce pattern
    for (int i = 0; i < num_partitions; i++)
    chess_loop_range(num_partitions, )
    {
        partial_denom_1[i] = aie::zeros<func_type, vector_size>(); // all the elements must be zeros, at the start
    }
    aie::vector<func_type, vector_size> partial_denom_2[num_partitions]; // init partial sums which contains the "map" phase of map-reduce pattern
    for (int i = 0; i < num_partitions; i++)
    chess_loop_range(num_partitions, )
    {
    ...
    int mean_2 = static_cast<func_type>(readincr_v<vector_size>(input_2)[0]);
    ...
    chess_prepare_for_pipelining
    {
        ...
        vec_2 = aie::add(convert_to_func_type(readincr_v<vector_size>(input_2)), -mean_2);

        partial_num[index] = aie::add(partial_num[index], aie::mul(vec_1, vec_2).to_vector<func_type>());
        partial_denom_1[index] = aie::add(partial_denom_1[index], aie::mul(vec_1, vec_1).to_vector<func_type>());
        partial_denom_2[index] = aie::add(partial_denom_2[index], aie::mul(vec_2, vec_2).to_vector<func_type>());
    }
